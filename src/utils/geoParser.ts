/**
 * Extracts Latitude and Longitude from various Google Maps links or raw coordinates
 */
...
export function parseGoogleMapsLinkOrCoords(input: string): ParsedLocation | null {
  ...
  // e.g. https://www.google.com/maps/place/.../@36.7538,3.0588,17z/...
  const atMatch = cleanInput.match(/@(-?\d+\.\d+),(-?\d+\.\d+)/);
  if (atMatch) {
    const lat = parseFloat(atMatch[1]);
    const lng = parseFloat(atMatch[2]);
    if (isValidAlgeriaLat(lat) && isValidAlgeriaLng(lng)) {
  ...
  // e.g. https://www.google.com/maps?q=36.7538,3.0588
  // e.g. https://maps.google.com/?daddr=36.7538,3.0588
  ...
  // 4. Check for decimal raw coordinates format e.g. "36.7538, 3.0588" or "36.7538 3.0588" or "36.7538,3.0588"
  ...
  // 5. Check for DMS (Degrees Minutes Seconds) e.g., 36°45'13.7"N 3°03'31.7"E
  ...
  // 6. Generic search for any two consecutive decimal numbers that look like coordinates
...
function isValidAlgeriaLat(lat: number): boolean {
  return lat >= 18.0 && lat <= 38.0;
}

function isValidAlgeriaLng(lng: number): boolean {
  return lng >= -9.0 && lng <= 13.0;
}
...
/**
 * Format distance to human-readable string (Arabic/French/English)
 */
export function formatDistance(distanceKm: number, lang: 'ar' | 'fr' | 'en' = 'ar'): string {
  if (distanceKm < 1) {
    const meters = Math.round(distanceKm * 1000);
    if (lang === 'ar') return `${meters} متر`;
    if (lang === 'fr') return `${meters} m`;
    return `${meters} m`;
  }
  if (lang === 'ar') return `${distanceKm} كم`;
  if (lang === 'fr') return `${distanceKm} km`;
...
/**
 * Build Google Maps Web View URL
 */
