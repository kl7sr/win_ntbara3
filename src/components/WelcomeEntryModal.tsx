...
export const WelcomeEntryModal: React.FC<WelcomeEntryModalProps> = ({
  ...
  const [selectedIntent, setSelectedIntent] = useState<'find' | 'add'>('find');
      ...
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[92vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4 text-right">
          {step === 1 ? (
            ...
            /* STEP 2: Location Step (Type-to-filter search + GPS) */
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                >
                  ...
                  <span>
                    {currentLanguage === 'ar' ? 'رجوع' : currentLanguage === 'fr' ? 'Retour' : 'Back'}
                  </span>
                </button>
              ...
              <button
                ...
                className="w-full p-3 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs flex items-center justify-center gap-2 shadow-sm transition active:scale-[0.99] disabled:opacity-75"
              >
                {isGpsLoading ? (
              ...
              <div className="relative flex py-1 items-center">
                ...
                <span className="flex-shrink mx-3 text-xs text-slate-400 font-semibold">
                  {currentLanguage === 'ar' ? 'أو ابحث بالاسم والرقم' : 'Ou recherchez'}
                </span>
                <div className="flex-grow border-t border-slate-200"></div>
              </div>

              {/* Live Type-to-Filter Search Box */}
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-50 border-2 border-slate-300 focus:border-emerald-600 focus:bg-white rounded-2xl px-4 py-3 pl-10 text-xs sm:text-sm text-slate-900 placeholder-slate-400 focus:outline-none transition font-medium"
                />
                <Search className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
              </div>

              {/* Wilayas Filtered Result List */}
